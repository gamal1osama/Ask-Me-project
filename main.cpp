...
public:
	...
	void LoadDatabase() {
		...
		vector<string> lines = ReadFileLines("project_questions.txt");
		for (const auto &line : lines) {
	...
	void UpdateDatabase() {
		...
		WriteFileLines("project_questions.txt", lines, false);
	}
};
...
public:
	...
	void LoadDatabase() {
		...
		vector<string> lines = ReadFileLines("project_users.txt");
		for (const auto &line : lines) {
			User user(line);
	...
	void UpdateDatabase(const User &user) {
		...
		WriteFileLines("project_users.txt", lines);
	}
